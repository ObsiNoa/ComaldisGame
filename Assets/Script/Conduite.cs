...
public class Conduite : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // --- ROTATION DU CAMION (remplace le strafe) ---
        // On normalise la vitesse pour moduler le braquage (pas de virage à l'arrêt, virage plein à vitesse max)
        float speedFactor = Mathf.Clamp(speed / maxForwardSpeed, -1f, 1f);
        float turnAmount = x * turnSpeed * speedFactor * Time.deltaTime;
        transform.Rotate(0f, turnAmount, 0f);

        // --- DÉPLACEMENT AVANT/ARRIÈRE UNIQUEMENT ---
